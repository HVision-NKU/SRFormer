...
class PSA(nn.Module):
    ...
    Args:
        ...
        proj_drop (float, optional): Dropout ratio of output. Default: 0.0
    """
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,squeeze_dim=45):
        ...
        self.kv = nn.Linear(dim, squeeze_dim*2, bias=qkv_bias)
        self.q = nn.Linear(dim,squeeze_dim*4,bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(squeeze_dim*4, dim)
        self.proj_drop = nn.Dropout(proj_drop)
    ...
    def forward(self, x, mask=None):
        ...
        kv = self.kv(x).reshape(b_,self.permuted_window_size[0],2,self.permuted_window_size[1],2,2,self.squeeze_dim).permute(0,1,3,5,2,4,6).reshape(b_, n//4, 2, self.num_heads, self.squeeze_dim*4 // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
        # keep the channel dimension of Q: (num_windows*b, num_heads, n, c//num_heads)
        q = self.q(x).reshape(b_, n, 1, self.num_heads, self.squeeze_dim*4 // self.num_heads).permute(2, 0, 3, 1, 4)[0]
        q = q * self.scale
        ...
        x = (attn @ v).transpose(1, 2).reshape(b_, n, self.squeeze_dim*4)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        for insert_num in [0,3,6]:
            tmp_modulelist.insert(insert_num,
        ...
        # build blocks
        self.blocks = nn.ModuleList(tmp_modulelist)
...
class SRFormerv2(nn.Module):
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(SRFormerv2, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
